...
def train_dino(args):
    ...
    fp16_scaler = None
    if args.use_fp16:
        fp16_scaler = torch.cuda.amp.GradScaler() if torch.cuda.is_available() else None
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader, optimizer, lr_schedule, wd_schedule, momentum_schedule, epoch, fp16_scaler, args):
    ...
    for it, (images, _) in enumerate(data_loader):
        ...
        with torch.amp.autocast(device_type=device.type, enabled=fp16_scaler is not None):
